...
main() {
    ...
    # Add to PATH (for zsh)
    if [[ -f "$HOME/.zshrc" ]] && ! grep -q "ubuntu-codespace/bin" "$HOME/.zshrc"; then
        echo '' >> "$HOME/.zshrc"
        echo "# Michael's Codespaces" >> "$HOME/.zshrc"
        echo "export PATH=\"\$HOME/.ubuntu-codespace/bin:\$PATH\"" >> "$HOME/.zshrc"
    fi
    
    # Add to PATH (for bash)
    if [[ -f "$HOME/.bashrc" ]] && ! grep -q "ubuntu-codespace/bin" "$HOME/.bashrc"; then
        echo '' >> "$HOME/.bashrc"
        echo "# Michael's Codespaces" >> "$HOME/.bashrc"
        echo "export PATH=\"\$HOME/.ubuntu-codespace/bin:\$PATH\"" >> "$HOME/.bashrc"
    fi
    ...
    printf "\n"
    printf "2. ${tty_bold}Check system health${tty_reset}:\n"
    printf "   $ mcs doctor\n"
    printf "\n"
    printf "3. ${tty_bold}Setup GitHub authentication${tty_reset}:\n"
    printf "   $ ~/codespaces/shared/scripts/setup-github-auth.sh\n"
    printf "\n"
    printf "4. ${tty_bold}Create your first codespace${tty_reset}:\n"
